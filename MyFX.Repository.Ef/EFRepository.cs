...
namespace MyFX.Repository.Ef
{
        ...
        : EntityBase<TKey>, IAggregateRoot<TKey>
    {
        ...
        public IQueryable<TEntity> Find(Expression<Func<TEntity, bool>> @where, Expression<Func<TEntity, object>> orderBy = null, bool isAsc = true)
        {
            if (where == null)
            {
                throw new ArgumentNullException("where");
            ...
            return isAsc ? queryObj.OrderBy(orderBy)
                : queryObj.OrderByDescending(orderBy);
        }

        public IQueryable<TEntity> Find(Expression<Func<TEntity, bool>> @where, params SortRule[] sortRules)
        {
            if (where == null)
            {
                throw new ArgumentNullException("where");
